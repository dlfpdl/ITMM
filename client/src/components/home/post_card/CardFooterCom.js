...
const CardFooterCom = ({ post }) => {
	...
	const handleLike = async () => {
		if (loadLike) return;
		setLoadLike(true);
		await dispatch(likeCom({ post, auth, socket }));
		setLoadLike(false);
	};
	...
	const handleUnLike = async () => {
		if (loadLike) return;
		setLoadLike(true);
		await dispatch(unLikeCom({ post, auth, socket }));
		setLoadLike(false);
	};
